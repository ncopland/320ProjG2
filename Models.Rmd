...
```
```{r}
franklin_arrests <- franklin_arrests %>%
  mutate(
    Hour = hour(Arrest_dttm),
    Month = month(Date),
    Day = day(Date),
...
```{r}
model_data <- franklin_arrests %>%
  select(
    Hour, Month, Day, Weekday, Is_Weekend, Season, Semester,
    UNC_in_session, Drugs_Alcohol, Arrest_Type, Age, Gender, Race,
    latitude, longitude
  ) %>%
...
```{r}
lm_model <- lm(Hour ~ ., data = model_data)
model_data$LM_Predictions <- predict(lm_model)
```

```{r}
library(randomForest)
rf_model <- randomForest(Hour ~ ., data = model_data, ntree = 100)
...
model_data1 = model_data %>%
  select(-LM_Predictions, -RF_Predictions)
trainIndex <- createDataPartition(model_data1$Hour, p = .8, list = FALSE)
train_data <- model_data1[trainIndex, ]
test_data  <- model_data1[-trainIndex, ]

ctrl <- trainControl(method = "cv", number = 5)
knn_model <- train(Hour ~ ., data = train_data, method = "knn", trControl = ctrl)
test_data$KNN_Predictions <- predict(knn_model, newdata = test_data)
```
...
mae_df <- bind_rows(
  evaluate_model(model_data$Hour, model_data$LM_Predictions) %>% mutate(Model = "Linear Regression"),
  evaluate_model(model_data$Hour, model_data$RF_Predictions) %>% mutate(Model = "Random Forest"),
  evaluate_model(test_data$Hour, test_data$KNN_Predictions) %>% mutate(Model = "KNN")
)
...
model_data_long <- model_data %>%
  pivot_longer(cols = ends_with("_Predictions"), names_to = "Model", values_to = "Prediction")

ggplot(model_data_long, aes(x = Prediction, y = Hour, color = Model)) +
  geom_point(alpha = 0.3) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  labs(title = "Predicted vs Actual Hour of Arrest on Franklin Street") +
  theme_minimal()
