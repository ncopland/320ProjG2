...
if (!dir.exists("monthly_maps")) dir.create("monthly_maps")
...
for(i in seq_len(nrow(year_months))) {
  y <- year_months$year[i]
  m <- year_months$month_num[i]
  
  # subset data for that month
  data_subset <- Arrests %>%
    filter(year == y, month_num == m)
  
  cat(sprintf("Rendering: %d-%02d with %d rows\n", y, m, nrow(data_subset)))
  
  
  if (nrow(data_subset) == 0) next  # skip if empty
  
 p <- ggplot() +
    geom_path(data = ch, aes(x = V1, y = V2), color = "black") +
    geom_point(data = data_subset,
               aes(x = longitude, y = latitude, color = Arrest_Type),
               alpha = 0.7, size = 1.5) +
    coord_quickmap(xlim = c(-79.09, -78.99), ylim = c(35.86, 35.99)) +
    labs(
      title = "Arrests in Chapel Hill",
      subtitle = sprintf("Year: %d  Month: %02d", y, m),
      x = "Longitude", y = "Latitude", color = "Arrest Type"
    ) +
    theme_minimal()
  
  ggsave(
    filename = sprintf("monthly_maps/arrests_%d_%02d.png", y, m),
...
pngs_sorted <- pngs %>%
  ...
  mutate(
    fname = basename(path),
    year = as.numeric(str_extract(fname, "\\d{4}")),
    month = as.numeric(str_extract(fname, "_(\\d{2})\\.png$", group = 1))
  ) %>%
  arrange(year, month) %>%
  pull(path)
...
gifski(
  ...
  gif_file = "arrests_map_animation_with_kind.gif",
  width = 1200,
  height = 800,
...
```
```{r, echo=FALSE, out.width='100%'}
knitr::include_graphics("arrests_map_animation_with_kind.gif")
```
...
```
```{r, echo=FALSE, out.width='100%'}
knitr::include_graphics("all_years_arrests.gif")
```
